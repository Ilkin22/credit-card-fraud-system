...
def create_app() -> Flask:
    ...
    def recent():
        """Return the last N decisions from the database (default 20, max 100)."""
        try:
    ...
    def stats():
        ...
        cur.execute("SELECT COUNT(*) FROM transactions WHERE risk_level = 'Critical'")
        critical = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM transactions WHERE risk_level = 'Suspicious'")
        suspicious = cur.fetchone()[0]
        cur.execute("SELECT AVG(probability) FROM transactions")
        avg_prob = cur.fetchone()[0] or 0.0
        cur.execute("SELECT AVG(amount) FROM transactions")
        ...
        return jsonify(
            ...
            safe=total - critical - suspicious,
